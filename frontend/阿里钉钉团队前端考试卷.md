...
不知道这道题是想问我如何把线上已有的资源重新搜集成本地的素材中心，还是单纯的把 显示资源换成本地的。我对这道题目的理解是：比如一个页面内有一个el-table，5000页，每页20个商品，你也不知道每个商品的链接，每个商品后面的URL还没有规律，此时对于开发者而言，你所知道的也就是页面的节点、接口的API这两个关键信息了。想要把这20*5000个商品的商品详情图重新整理本地素材中心，那就手写一个爬虫，一次性爬完，把所有的资源路径拿下来。或者写一个chrome一键收集。
