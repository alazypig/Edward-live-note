...
这种在内存中构建并直接替换的技术叫做[双缓存 (opens new window)](https://baike.baidu.com/item/%E5%8F%8C%E7%BC%93%E5%86%B2)。
...
![fiberRoot](笔记/frontend/images/8.png)
...
![workInProgressFiber](笔记/frontend/images/9.png)
...
![workInProgressFiberFinish](笔记/frontend/images/10.png)

## update时

1. 接下来我们点击`p节点`触发状态改变，这会开启一次新的`render阶段`并构建一棵新的`workInProgress Fiber 树`。

![wipTreeUpdate](笔记/frontend/images/11.png)
...
![currentTreeUpdate](笔记/frontend/images/12.png)

## 总结

本文介绍了`Fiber树`的构建与替换过程，这个过程伴随着`DOM`的更新。

那么在构建过程中每个`Fiber节点`具体是如何创建的呢？我们会在`架构篇`的[render阶段](https://react.iamkasong.com/process/reconciler.html)讲解。